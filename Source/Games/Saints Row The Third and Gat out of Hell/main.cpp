#define GAME_SAINTS_ROW_THE_THIRD 1
...
namespace
{
   ...
   // The frame's first rl_downsample_02 is the quarter-res brightpass source (960x540 at 4K) (16-tap box * its vc4 Tint_color, b6).
...
struct SaintsRowTheThirdGameDeviceData final : public GameDeviceData
{
   ...
   // Tint_color (measured 0.20-0.33, grey), applied only in the composite, so the MSAA scene is unexposed; the weighted
   // resolve reads last frame's copy to weight samples in the displayed exposure. Copied only while Luma MSAA is on; null
   // until then (the resolve shader falls back to a box resolve on the zeroed binding).
   ...
   {
      const char* name;
      com_ptr<ID3D11Buffer> SaintsRowTheThirdGameDeviceData::* source;
      UINT register_index;
   ...
   BloomConstantReadout bloom_readouts[6] = {
      ...
      {"Combine Tint_color (vc4 c1)", &SaintsRowTheThirdGameDeviceData::bloom_combine_vc4_cb, 1},
   };
...
class SaintsRowTheThird final : public Game
{
   static SaintsRowTheThirdGameDeviceData& GetGameDeviceData(DeviceData& device_data)
   {
      return *static_cast<SaintsRowTheThirdGameDeviceData*>(device_data.game);
   }

public:
   void OnCreateDevice(ID3D11Device* native_device, DeviceData& device_data) override
   {
      device_data.game = new SaintsRowTheThirdGameDeviceData;
   }

   void OnDestroyDeviceData(DeviceData& device_data) override
   {
      // GameDeviceData lacks a virtual destructor; delete through the concrete type to release derived members.
      delete static_cast<SaintsRowTheThirdGameDeviceData*>(device_data.game);
   ...
   DrawOrDispatchOverrideType DrawTonemapWithSMAA(ID3D11Device* native_device, ID3D11DeviceContext* native_device_context, CommandListData& cmd_list_data, DeviceData& device_data, bool& updated_cbuffers, const std::function<void()>& original_draw_dispatch_func, UINT depth_slot)
   {
...

      {
         ...
         native_device_context->CSSetShader(device_data.native_compute_shaders.at("SR3 SMAA Linearize CS"_h).get(), nullptr, 0);
         ...
         if (depth_srv)
         {
            ...
            native_device_context->CSSetShader(device_data.native_compute_shaders.at("SR3 SMAA Predication CS"_h).get(), nullptr, 0);
            native_device_context->Dispatch((canvas_desc.Width + 7) / 8, (canvas_desc.Height + 7) / 8, 1);
         }
      ...
      const bool sharpen = g_rcas_sharpness > 0.f && HasShaders(device_data.native_vertex_shaders, "Copy VS"_h) && HasShaders(device_data.native_pixel_shaders, "SR3 Sharpen PS"_h);
      ...
      if (sharpen)
      {
         ...
         DrawCustomPixelShader(native_device_context, device_data.default_depth_stencil_state.get(), device_data.default_blend_state.get(), nullptr, device_data.native_vertex_shaders.at("Copy VS"_h).get(), device_data.native_pixel_shaders.at("SR3 Sharpen PS"_h).get(), game_device_data.smaa_gamma_srv.get(), canvas_rtv.get(), canvas_desc.Width, canvas_desc.Height, false);
   ...
   bool RunXeGTAO(ID3D11Device* native_device, ID3D11DeviceContext* native_device_context, DeviceData& device_data)
   {
      ...
      if (!HasShaders(shaders, "SR3 XeGTAO Prefilter Depths CS"_h, "SR3 XeGTAO Main Pass CS"_h, "SR3 XeGTAO Denoise Pass 1 CS"_h, "SR3 XeGTAO Denoise Pass 2 CS"_h))
         return false;
      ...
      pass("SR3 XeGTAO Prefilter Depths CS"_h, 5, mip_uavs, {depth_srv.get(), nullptr}, (width + 15) / 16, (height + 15) / 16);
      pass("SR3 XeGTAO Main Pass CS"_h, 1, &working_uavs[0], {game_device_data.gtao_depth_mips_srv.get(), normals_srv.get()}, (width + 7) / 8, (height + 7) / 8);
      pass("SR3 XeGTAO Denoise Pass 1 CS"_h, 1, &working_uavs[1], {game_device_data.gtao_working_srvs[0].get(), nullptr}, (width + 15) / 16, (height + 7) / 8);
      pass("SR3 XeGTAO Denoise Pass 2 CS"_h, 1, &final_uav, {game_device_data.gtao_working_srvs[1].get(), nullptr}, (width + 15) / 16, (height + 7) / 8);
   ...
   DrawOrDispatchOverrideType OnDrawOrDispatch(ID3D11Device* native_device, ID3D11DeviceContext* native_device_context, CommandListData& cmd_list_data, DeviceData& device_data, reshade::api::shader_stage stages, const ShaderHashesList<OneShaderPerPipeline>& original_shader_hashes, bool is_custom_pass, bool& updated_cbuffers, std::function<void()>* original_draw_dispatch_func) override
   {
      ...
      if (copy_bloom_constants && pixel_shader_hash == bloom_combine_pixel_shader)
      {
      ...
      if (copy_bloom_constants && !game_device_data.bloom_source_downsampled && pixel_shader_hash == downsample_pixel_shader)
      {
   ...
   // Luma_SR3_MSAAResolve instead. Anything else, or a target we cannot bind, keeps the hardware resolve.
   static bool OnResolveTextureRegion(reshade::api::command_list* cmd_list, reshade::api::resource source, uint32_t source_subresource, const reshade::api::subresource_box* source_box, reshade::api::resource dest, uint32_t dest_subresource, uint32_t dest_x, uint32_t dest_y, uint32_t dest_z, reshade::api::format format)
   {
      ...
      const std::shared_lock lock_shader_objects(s_mutex_shader_objects);
      if (!HasShaders(device_data.native_vertex_shaders, "Copy VS"_h) || !HasShaders(device_data.native_pixel_shaders, "SR3 MSAA Resolve PS"_h))
         return false;
      ...
      DrawCustomPixelShader(native_device_context, device_data.default_depth_stencil_state.get(), device_data.default_blend_state.get(), nullptr, device_data.native_vertex_shaders.at("Copy VS"_h).get(), device_data.native_pixel_shaders.at("SR3 MSAA Resolve PS"_h).get(), source_srv.get(), dest_rtv.get(), dest_desc.Width, dest_desc.Height);
   ...
   void OnInit(bool async) override
   {
      ...
      // The tonemap writes gamma-encoded output and DoF, distortion and UI keep running on it after,
      ...
      native_shaders_definitions.emplace(CompileTimeStringHash("SR3 Sharpen PS"), ShaderDefinition{"Luma_SR3_Sharpen", reshade::api::pipeline_subobject_type::pixel_shader, nullptr, "sharpen_ps"});
      // XeGTAO passes (Luma_SR3_XeGTAO.hlsl); the two denoisers differ only by XE_GTAO_FINAL_APPLY.
      native_shaders_definitions.emplace(CompileTimeStringHash("SR3 XeGTAO Prefilter Depths CS"), ShaderDefinition{"Luma_SR3_XeGTAO", reshade::api::pipeline_subobject_type::compute_shader, nullptr, "prefilter_depths16x16_cs"});
      native_shaders_definitions.emplace(CompileTimeStringHash("SR3 XeGTAO Main Pass CS"), ShaderDefinition{"Luma_SR3_XeGTAO", reshade::api::pipeline_subobject_type::compute_shader, nullptr, "main_pass_cs"});
      native_shaders_definitions.emplace(CompileTimeStringHash("SR3 XeGTAO Denoise Pass 1 CS"), ShaderDefinition{"Luma_SR3_XeGTAO", reshade::api::pipeline_subobject_type::compute_shader, nullptr, "denoise_pass_cs", {{"XE_GTAO_FINAL_APPLY", "0"}}});
      native_shaders_definitions.emplace(CompileTimeStringHash("SR3 XeGTAO Denoise Pass 2 CS"), ShaderDefinition{"Luma_SR3_XeGTAO", reshade::api::pipeline_subobject_type::compute_shader, nullptr, "denoise_pass_cs", {{"XE_GTAO_FINAL_APPLY", "1"}}});
...
BOOL APIENTRY DllMain(HMODULE hModule, DWORD ul_reason_for_call, LPVOID lpReserved)
{
   ...
   {
      reshade::unregister_event<reshade::addon_event::create_resource>(SaintsRowTheThird::OnCreateResource);
      reshade::unregister_event<reshade::addon_event::resolve_texture_region>(SaintsRowTheThird::OnResolveTextureRegion);
   }
