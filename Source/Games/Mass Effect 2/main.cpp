// Mass Effect 2 (2010) Luma HDR mod: UE3, 32-bit, DX9 -> D3D11 via dgVoodoo2 2.87.3 (2.81.3 hashes kept alongside).
...
// BloomScaleLive gain alone makes Bloom Intensity 1 vanilla strength; the wider shape is deliberate.
...
struct MassEffect2GameDeviceData final : public GameDeviceData
{
   ...
   // A canvas is display-mapped, encoded and pre-scaled: by the material, the material-less display map or the
   // gamma pass. Also what the gamma replacement is told, so it never maps a finished canvas twice.
   bool has_finished_canvas = false;
   ...
   // vanilla picture, dead sliders, not one error. Sticky across frames, unlike the per-frame latches above.
   bool ever_matched_keyed_pass = false;
   uint32_t frames_presented = 0; // counts up to the check frame, then stops
#if DEVELOPMENT
   ...
   // recognise later draws targeting it) and by the SMAA hook. Released every Present.
   ComPtr<ID3D11Resource> canvas_res;

   // The uber's target until a canvas is finished; still set when a later draw targets it = material-less frame.
   ...
   // these back into a shader, so this whole block is absent from a shipping build. `grade_logged` is the last SET the log
   ...
   // Turning the feature off must give the address space back: ~132 MB at 4K here, plus core's SMAA intermediates.
...
class MassEffect2Game final : public Game
{
   ...
   // Pass identity by shader hash, folding both supported dgVoodoo builds so every hook below keys on either (ME1's
   // shape). Each Is*Pass test also folds every perm of its pass: an unmatched one would cost the whole Luma tail on
   // that frame (it cost TW2 its HDR block, SMAA and Hide-UI gate at once).
   ...
   // on a normal frame the canvas capture and SMAA key on it rather than on the uber.
   ...
   // UE3 FGammaCorrection: a scene pass too, never HUD, so neither Hide UI nor the material-less map may take it.
   static bool IsGammaCorrectionPass(const ShaderHashesList<OneShaderPerPipeline>& hashes)
   {
   ...
   // `row_count` consecutive rows of the bound PS b4 (dgVoodoo's constant mirror) from the copy made kSlots - 1
   // captures ago, all out of the SAME mapped slot, so a multi-row read stays one capture and one Map. Deferred so the Map
   // never stalls. `out` is written only once every early-out has passed. False when there is nothing to read yet:
   // fresh ring, failed allocation, or a slot still in flight.
   static bool ReadCB4RowsDeferred(ID3D11Device* native_device, ID3D11DeviceContext* native_device_context,
      MassEffect2GameDeviceData::DeferredCBRing& ring, uint32_t first_row, uint32_t row_count, float* out)
   {
      ...
      native_device_context->CopyResource(ring.staging[ring.writes % kSlots].get(), cb.get());
      ring.writes++;
      ring.advanced_this_frame = true;
      if (ring.writes < kSlots)
         return false; // nothing old enough to read yet

      // The slot about to be overwritten next is the oldest one, i.e. the copy issued kSlots - 1 captures ago.
   ...
   // silhouettes are ~1% of a frame: tens of percent = tolerance too low, ~0 = too high. One-shot, the Map stalls.
...
public:
   void OnInit(bool async) override
   {
      ...
      native_shaders_definitions.emplace(CompileTimeStringHash("ME2 SMAA Linearize CS"),
      ...
      // The canvas stores gamma so the game's UE3 Canvas HUD blends onto it exactly as
      ...
      // Seed for the frames before the gather's first readback: the engine drives this per zone over a ~5x range, and
      // 1.0 is the highest value logged.
   ...
   void DrawImGuiSettings(DeviceData& device_data) override
   {
      ...
      // --- Grade (read in Luma_ME2_Tonemap.hlsl via LumaSettings.GameSettings). HDR tonemap path only except Exposure,
      // Dithering and the two native-look controls. ---
      auto& gs = cb_luma_global_settings.GameSettings;
      ...
      if (DrawResetButton(gs.FilmGrainIntensity, default_luma_global_game_settings.FilmGrainIntensity, "FilmGrainIntensity"))
         device_data.cb_luma_global_settings_dirty = true;
   ...
   void OnCreateDevice(ID3D11Device* native_device, DeviceData& device_data) override
   {
      ...
      // UI) must still count or composition treats the frame as unfinished. Not latched per frame at FGammaCorrection
      // as in ME1: that pass has never been seen drawing here.
   ...
   void UpdateLumaInstanceDataCB(CB::LumaInstanceDataPadded& data, CommandListData& cmd_list_data, DeviceData& device_data) override
   {
      ...
      data.GameData.UberRanThisFrame = game_device_data.has_drawn_uber ? 1.f : 0.f;
      data.GameData.CanvasFinishedThisFrame = game_device_data.has_finished_canvas ? 1.f : 0.f;
   }
...
#if ENABLE_SMAA
   // SMAA on the finished gamma canvas, before the UE3 Canvas HUD draws onto it: called after the material, the
   // material-less display map or the gamma pass. ME1 2007 chain: snapshot -> linear decode (+ predication mask) ->
   // DrawSMAA -> optional RCAS, the last pass writing the canvas RTV. No-op while anything is missing.
   void RunPostMaterialSMAA(ID3D11Device* native_device, ID3D11DeviceContext* native_device_context, DeviceData& device_data, MassEffect2GameDeviceData& gd, ID3D11Resource* canvas_res, ID3D11RenderTargetView* canvas_rtv)
   {
      ...
      if (!gd.tex_input_linear)
      {
         const CD3D11_TEXTURE2D_DESC td(DXGI_FORMAT_R16G16B16A16_FLOAT, w, h, 1, 1, D3D11_BIND_SHADER_RESOURCE | D3D11_BIND_UNORDERED_ACCESS);
         if (SUCCEEDED(native_device->CreateTexture2D(&td, nullptr, gd.tex_input_linear.put())))
         {
      ...
      // One compute state stack for both dispatches (MoHA's shape).
      {
         DrawStateStack<DrawStateStackType::Compute> cs_state;
         cs_state.Cache(native_device_context, device_data.uav_max_count);

         // Linear-light decode of the snapshot for the neighborhood blend (Luma_ME2_SMAALinearize.hlsl).
         ...
         if (pred_ok)
         {
            ...
            // UAV first: an SRV set while its resource is still bound as a UAV is silently nulled. Slot 0 of both
            // still holds the linearize views here, which alias nothing of the predication pair.
   ...
   DrawOrDispatchOverrideType OnDrawOrDispatch(ID3D11Device* native_device, ID3D11DeviceContext* native_device_context, CommandListData& cmd_list_data, DeviceData& device_data, reshade::api::shader_stage stages, const ShaderHashesList<OneShaderPerPipeline>& original_shader_hashes, bool is_custom_pass, bool& updated_cbuffers, std::function<void()>* original_draw_dispatch_func) override
   {
      ...
      if (is_immediate && game_device_data.uber_rt_res && (!is_custom_pass || original_shader_hashes.pixel_shaders[0] != UINT64_MAX) && !IsMaterialPass(original_shader_hashes) && !IsGammaCorrectionPass(original_shader_hashes))
      {
         ...
         if (rt_res && rt_res.get() == game_device_data.uber_rt_res.get())
         {
            ...
            if (GetShaderDefineCompiledNumericalValue(char_ptr_crc32("TONEMAP_TYPE")) >= 1 && copy_vs && map_ps)
            {
               ...
               GetResourceInfo(rt_res.get(), info, fmt);
               ...
               if (!game_device_data.tex_display_map)
               {
                  game_device_data.tex_display_map = CloneTexture<ID3D11Texture2D>(native_device, rt_res.get(), typed_fmt, D3D11_BIND_SHADER_RESOURCE, D3D11_BIND_RENDER_TARGET, false, false).get();
                  if (game_device_data.tex_display_map)
                     native_device->CreateShaderResourceView(game_device_data.tex_display_map.get(), nullptr, game_device_data.srv_display_map.put());
               }
               if (game_device_data.srv_display_map)
               {
                  native_device_context->CopyResource(game_device_data.tex_display_map.get(), rt_res.get());
      ...
      // Hide UI: cancel draws after the finished canvas that target it - the render-target test is load-bearing, since
      // "everything after" also swallows the present blit.
      if (g_hide_ui && is_immediate && (!is_custom_pass || original_shader_hashes.pixel_shaders[0] != UINT64_MAX) && game_device_data.canvas_res && !IsGammaCorrectionPass(original_shader_hashes))
      {
      ...
      {
         // The engine's BloomScale (cb4[11].x). Implausible data is rejected, keeping the last good value.
         float row[4];
         if (ReadCB4RowsDeferred(native_device, native_device_context, game_device_data.bloom_cb_ring, 11, 1, row) && row[0] >= 0.f && row[0] <= 4.f)
         {
            game_device_data.bloom_scale_live = row[0];
            game_device_data.bloom_scale_valid = true; // separate from the value: 0 is a legitimate reading (volume with bloom off)
         }
      }
#endif
      ...
      if (is_immediate && !game_device_data.has_drawn_uber && IsUberPass(original_shader_hashes))
      {
...
#if DEVELOPMENT
         // Grade rows off this draw's b4, before anything below rebinds; only logged (see OnPresent). Its own ring, so it
         // cannot fight the gather's for the one capture a frame each ring is allowed. No plausibility window, unlike
         // BloomScale: rejecting a reading would hide exactly the odd zone worth recording.
         if (ReadCB4RowsDeferred(native_device, native_device_context, game_device_data.grade_cb_ring, kGradeFirstRow, kGradeRowCount, &game_device_data.grade_rows[0][0]))
            game_device_data.grade_valid = true;
#endif
         ...
         // overwrites it before the material draws. Not gated on is_custom_pass, which a replaced uber also sets.
      ...
      if (is_immediate && !game_device_data.has_finished_canvas && IsMaterialPass(original_shader_hashes))
      {
         ...
         // callback core issues the draw as usual (no AA) rather than the encode being skipped.
      ...
      // UE3 FGammaCorrection, never yet seen drawing. Its replacement branches on what ran before it (see
      // UpdateLumaInstanceDataCB). On a frame with no finished canvas it finishes one itself, so the draw is issued
      // from here: LumaData is pushed BEFORE the latch below flips (returning Replaced skips core's upload), and the
      // canvas is captured for Hide UI and SMAA before the HUD. After a finished canvas it is left to core, which
      // uploads the unchanged flags.
      if (is_immediate && IsGammaCorrectionPass(original_shader_hashes))
      {
         game_device_data.ever_matched_keyed_pass = true;
#if DEVELOPMENT
         if (!game_device_data.diag_logged_gamma)
         {
            game_device_data.diag_logged_gamma = true;
            char msg[160];
            std::snprintf(msg, sizeof(msg), "[Luma] ME2 DIAG: FGammaCorrection drew (uber ran %d, canvas finished %d)", (int)game_device_data.has_drawn_uber, (int)game_device_data.has_finished_canvas);
            reshade::log::message(reshade::log::level::info, msg);
         }
#endif
         ComPtr<ID3D11RenderTargetView> canvas_rtv;
         native_device_context->OMGetRenderTargets(1, canvas_rtv.put(), nullptr);
         if (!game_device_data.has_finished_canvas && canvas_rtv && original_draw_dispatch_func != nullptr)
         {
            SetLumaConstantBuffers(native_device_context, cmd_list_data, device_data, reshade::api::shader_stage::pixel, LumaConstantBufferType::LumaSettings);
   ...
   void OnPresent(ID3D11Device* native_device, DeviceData& device_data) override
   {
      ...
      // One line per distinct grade, so walking between post-process volumes writes the set down without a capture.
      // The comparison is against what was last LOGGED, not against the previous frame: the engine interpolates
      ...
      if (game_device_data.grade_valid)
      {
         ...
         for (uint32_t r = 0; r < kGradeRowCount && !changed; r++)
            for (uint32_t c = 0; c < 4; c++)
            {
               ...
               // transition, which printed the whole ramp. Its .w, the display
         ...
         if (changed)
         {
            ...
            // Row names are the uber's register map (Luma_ME2_Tonemap.hlsl). Row 15 doubles as the engine fade, so a reading
            ...
            for (uint32_t r = 0; r < kGradeRowCount; r++)
            {
               ...
               std::snprintf(msg, sizeof(msg), "[Luma] ME2 DIAG:   cb4[%s] = %.6f, %.6f, %.6f, %.6f", kRowNames[r],
                  game_device_data.grade_rows[r][0], game_device_data.grade_rows[r][1],
                  game_device_data.grade_rows[r][2], game_device_data.grade_rows[r][3]);
               reshade::log::message(reshade::log::level::info, msg);
            }
         }
      }
      ...
      // Release core's Karis views when the pyramid is off, on the render thread (DrawBloom's mips stay, see
      // ReleaseCoreKarisAverage). Latched so the two map lookups do not run every frame.
      if (!g_luma_bloom_enable && !game_device_data.karis_released)
         ReleaseCoreKarisAverage(device_data);
      game_device_data.karis_released = !g_luma_bloom_enable; // DrawKarisAverage recreates lazily once re-enabled
#endif
   ...
   {
      // Same text as the sibling dgVoodoo ports but for the game name. ShellExecuteA rather than
...
BOOL APIENTRY DllMain(HMODULE hModule, DWORD ul_reason_for_call, LPVOID lpReserved)
{
   if (ul_reason_for_call == DLL_PROCESS_ATTACH)
   {
      ...
      // the game. Keyed by format, not hash; the size filters are load-bearing in a 32-bit process.
      texture_format_upgrades_type = TextureFormatUpgradesType::AllowedEnabled;
      enable_indirect_texture_format_upgrades = true;
      enable_chain_indirect_texture_format_upgrades = ChainTextureFormatUpgradesType::DirectDependencies;
      texture_upgrade_formats = {
         reshade::api::format::r8g8b8a8_typeless,     // the two full-res canvases: uber target and material target
