...
float4 smaa_neighborhood_blending_ps(float4 position : SV_Position, float2 texcoord : TEXCOORD0, float4 offset : TEXCOORD1) : SV_Target
{
   ...
   // compressed so 1px-thin dark features survive the average (a linear blend erodes them). No HDR tail and no
   // re-encode: the output stays in the gamma canvas' space, mid-pipeline before the HUD and the composition.
