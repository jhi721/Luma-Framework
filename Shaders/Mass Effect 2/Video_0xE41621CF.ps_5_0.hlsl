// Mass Effect 2 (2010) Bink movie pass (YUV->RGB): SDR clamp + light AutoHDR. Movies never touch the scene passes,
// so without this a cutscene sits flat at paper white. Body from the 0xE41621CF disasm.
...
// Light AutoHDR on movies (0 = off). Peak kept low: Bink is low-bitrate and peak amplifies block artifacts. Gated on
// TONEMAP_TYPE, not display mode - the SDR self-noop does not cover TONEMAP_TYPE 0 on scRGB, where it reaches 2.06x.
    ...
    out float4 o0 : SV_TARGET0)
{
...
#if ENABLE_VIDEO_AUTO_HDR && TONEMAP_TYPE >= 1
   ...
   // not carry one either. Accessors, not LumaSettings - a deliberate divergence from ME1 2007: they carry the
   // HDR_TONEMAP_* overrides and the devkit white level. The zero guard has to stay.
