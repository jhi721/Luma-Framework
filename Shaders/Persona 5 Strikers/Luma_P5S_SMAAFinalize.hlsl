// Last pass of the SMAA chain when RCAS is on (without it SMAA writes the canvas itself), from the gamma SMAA output into the linear
// canvas: RCAS (on the gamma color, so the sharpening is perceptual), the decode, then the Luma dither the composite skipped. The UI draws after this.
// clang-format off
...
{
   // RCAS normalizes by the range's peak: the display's peak relative to UI paper white, gamma encoded like the input
