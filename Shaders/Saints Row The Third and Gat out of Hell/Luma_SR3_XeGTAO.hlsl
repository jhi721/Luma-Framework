...
#define EFFECT_FALLOFF_RANGE 0.615 // Default 0.615; the native term also fades with distance (1 / (1 + dist))
#endif
...
void XeGTAO_PrefilterDepths16x16(uint2 dispatchThreadID, uint2 groupThreadID, Texture2D sourceNDCDepth, RWTexture2D<float> outDepth0, RWTexture2D<float> outDepth1, RWTexture2D<float> outDepth2, RWTexture2D<float> outDepth3, RWTexture2D<float> outDepth4)
{
   ...
   // Explicit integer Loads instead of GatherRed, which returns 0 on some D24 views (flat depth -> AO silently gone).
   // Each target pixel takes the top-left full-res pixel of its block, the one the native first draw reads.
   const uint depthInputScale = uint(DepthInputScaleRT);
   float d00 = sourceNDCDepth.Load(int3((pixCoord + uint2(0, 0)) * depthInputScale, 0)).x;
   float d10 = sourceNDCDepth.Load(int3((pixCoord + uint2(1, 0)) * depthInputScale, 0)).x;
   float d01 = sourceNDCDepth.Load(int3((pixCoord + uint2(0, 1)) * depthInputScale, 0)).x;
   float d11 = sourceNDCDepth.Load(int3((pixCoord + uint2(1, 1)) * depthInputScale, 0)).x;
...
void XeGTAO_MainPass(uint2 pixCoord, float2 localNoise, float3 viewspaceNormal, Texture2D sourceViewspaceDepth, SamplerState depthSampler, RWTexture2D<unorm float2> outWorkingAOTermAndEdges)
{
   ...
   // see "Algorithm 1" in https://www.activision.com/cdn/research/Practical_Real_Time_Strategies_for_Accurate_Indirect_Occlusion_NEW%20VERSION_COLOR.pdf
   {
      ...
      // No occlusion floor here (Intel's 0.03 exists for bent-normal packing, unused): the native apply keeps its own 0.05.
   }
...
)
{
   ...
   if (DebugViewRT > 0.5)
   {
      for (int dside = 0; dside < 2; dside++)
      {
         ...
         outputTexture[dpix] = 1.0 - v; // the apply multiplies the scene by 1 - amount, so the scene shows v (floored at 0.05)
#else
...
#if XE_GTAO_FINAL_APPLY
      // The native apply averages the four channels (one per native draw) and takes 1 - that: amount, not visibility.
      outputTexture[pixCoord] = 1.0 - saturate(aoTerm[side] * XE_GTAO_OCCLUSION_TERM_SCALE);
#else
...
SamplerState smp : register(s0); // point-clamp, bound by Luma
...
[numthreads(8, 8, 1)] // <- hard coded to 8x8; each thread computes 2x2 blocks so processing 16x16 block: Dispatch needs to be called with (width + 16-1) / 16, (height + 16-1) / 16
    void prefilter_depths16x16_cs(uint2 dtid : SV_DispatchThreadID, uint2 gtid : SV_GroupThreadID) {
       // tex0 = the game's full-res hardware depth (the calculate's t14)
        ...
        [numthreads(XE_GTAO_NUMTHREADS_X, XE_GTAO_NUMTHREADS_Y, 1)] void main_pass_cs(uint2 dtid : SV_DispatchThreadID)
{
   ...
   // The game's Lambert azimuthal decode (rl_ssao_singleframe_calculate): e = t*2-1, f = |e|^2,
