// Saints Row: The Third and Gat out of Hell - the shared fp16 pyramidal bloom, built from the rl_hdr final's own scene
// input (t0) and bound over the game's Bloom_stage_0 (SR3 t5) / Final_bloom (GooH t6). The prefilter replays the native
// brightpass (SR3 rl_hdr_12, GooH rl_hdr_prep_08) and combine (rl_hdr_11 / rl_hdr_prep_07) on their own constants, copied at their draws earlier in the frame (main.cpp), with the tint of
...
{
#if GAT_OUT_OF_HELL // Set on the prefilter by main.cpp
...
// SR3 rl_hdr_12 0x87F73DF4, transcribed: S = 0.3 * (r + g + b), an equal-weight channel sum; below K.x the weight is
// linear (S * K.y), above it K.z * |K.x - S| + K.w; the colour is rescaled to that weight over S and tripled. Then the
// combine's tint. GooH rl_hdr_prep_08 0x12F9FD30 is the same with the input capped at 32 after its tint.
float3 sr_bloom_threshold(float3 color)
{
   ...
   color *= Source_downsample_tint.rgb * Brightpass_tint.w;
#if GAT_OUT_OF_HELL
   color = min(color, 32.0);
#endif
...
#define LUMA_BLOOM_THRESHOLD_FUNCTION(color) sr_bloom_threshold(color)
