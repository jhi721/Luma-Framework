// Saints Row: The Third - the shared fp16 pyramidal bloom, built from the rl_hdr final's own scene input (t0) and bound
// over the game's Bloom_stage_0 (t5). The prefilter replays the native rl_hdr_12 brightpass
// and rl_hdr_11 combine on their own constants, copied at their draws earlier in the frame (main.cpp), with the tint of
...
// rl_hdr_12 0x87F73DF4, transcribed: S = 0.3 * (r + g + b), an equal-weight channel sum; below K.x the weight is linear
// (S * K.y), above it K.z * |K.x - S| + K.w; the colour is rescaled to that weight over S and tripled. Then rl_hdr_11's tint.
float3 sr3_bloom_threshold(float3 color)
{
...
#define LUMA_BLOOM_THRESHOLD_FUNCTION(color) sr3_bloom_threshold(color)
