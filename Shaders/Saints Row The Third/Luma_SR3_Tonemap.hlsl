...
float3 SR3_SaturateAndTint(float3 scene)
{
...
float SR3_Shoulder(float u)
{
   return 1.5 * u - 0.5 * u * u * u;
}
float3 SR3_Shoulder(float3 u)
{
   return 1.5 * u - 0.5 * u * u * u;
}
...
float3 SR3_SampleLUT(float3 index)
{
...
float3 SR3_DisplayMap(float3 hdrLinear, float2 uv)
{
   ...
   float3 encoded = SR3_EncodeOutput(DICETonemap(hdrLinear * paperWhite, peakWhite, settings) / paperWhite);
...
float4 SR3_Output(float3 vanillaLinear, float recoveryGain, float2 uv, float alpha)
{
#if TONEMAP_TYPE <= 0
   return float4(SR3_EncodeOutput(vanillaLinear), alpha);
#else
   return float4(SR3_DisplayMap(Saturation(vanillaLinear, LumaSettings.GameSettings.Saturation) * recoveryGain, uv), alpha);
#endif
}
...
float3 SR3_ShoulderInput(float4 scene, float2 uv)
{
   ...
   return (SR3_SaturateAndTint(scene.rgb) + bloom) * LumaSettings.GameSettings.Exposure * (1.0 / 1.49);
}
...
// gives 0.36): here SDR white recovers 1.36x and scene 10 reaches ~8.9x paper white before DICE. MELE / RenoDX
...
float SR3_RecoveryGain(float3 u)
{
   ...
   if (sourcePeak <= pivot)
      return 1.0;
   const float extended = SR3_Shoulder(pivot) + 1.5 * (1.0 - pivot * pivot) * (sourcePeak - pivot);
   // max(1, ...) holds "never darker than vanilla" through float rounding at the pivot, and turns NaN into vanilla.
   return max(1.0, extended / SR3_Shoulder(min(sourcePeak, 1.0)));
}
...
float4 SR3_TonemapLUT(float2 uv)
{
   ...
   const float3 index = sqrt(SR3_Shoulder(saturate(u)));
   // The LUT's identity returns its own index, so the grading fades out in the encoded space it was authored in.
   const float3 vanilla = lerp(index, SR3_SampleLUT(index), LumaSettings.GameSettings.ColorGradingIntensity);
   return SR3_Output(gamma_to_linear(vanilla), SR3_RecoveryGain(u), uv, scene.a);
}
...
{
   const float4 scene = Base_samplerTexture.Sample(Base_samplerSampler, uv);
   const float3 color = SR3_SaturateAndTint(scene.rgb) * LumaSettings.GameSettings.Exposure;
   return SR3_Output(color, 1.0, uv, scene.a);
}
