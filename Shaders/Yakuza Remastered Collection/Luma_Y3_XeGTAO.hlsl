...
#define EFFECT_RADIUS (assao[5].x / RADIUS_MULTIPLIER) // anchored to ASSAO's live EffectRadius (0.8 view units, measured constant), so the effective XeGTAO radius equals the native one. RadiusOverrideRT > 0 wins.
#endif
...
#define EFFECT_FALLOFF_RANGE 0.615 // Default 0.615; ASSAO's own sample weight also falls off smoothly (1 - d^2 / r^2)
#endif
...
// ASSAO's unpack: viewZ = DepthUnpackConsts.x / (DepthUnpackConsts.y - d), measured (0.10001, 1.0001): near 0.1, far 1000.
float XeGTAO_ScreenSpaceToViewSpaceDepth(const float screenDepth)
{
   return assao[1].x / max(assao[1].y - screenDepth, 1e-6);
}
...
void XeGTAO_PrefilterDepths16x16(uint2 dispatchThreadID, uint2 groupThreadID, Texture2D sourceNDCDepth, RWTexture2D<float> outDepth0, RWTexture2D<float> outDepth1, RWTexture2D<float> outDepth2, RWTexture2D<float> outDepth3, RWTexture2D<float> outDepth4)
{
   ...
   // Explicit integer Loads of the 2x2 (the game's depth is an R32_FLOAT_X8X24 view of a depth-stencil texture).
...
#if DEVELOPMENT
   // Debug views (visible on screen through the apply, multiplied onto the scene; the final denoise passes raw values
   // through when DebugViewRT > 0).
...
#if XE_GTAO_GENERATE_NORMALS
   // Derive the view-space normal from the depth cross, edge-weighted (Intel reference path).
   {
...
[numthreads(8, 8, 1)] // <- hard coded to 8x8; each thread computes 2x2 blocks so processing 16x16 block: Dispatch needs to be called with (width + 16-1) / 16, (height + 16-1) / 16
    void prefilter_depths16x16_cs(uint2 dtid : SV_DispatchThreadID, uint2 gtid : SV_GroupThreadID) {
       // tex0 = the game's full-res r32 hardware depth (the ASSAO prepare's t0)
