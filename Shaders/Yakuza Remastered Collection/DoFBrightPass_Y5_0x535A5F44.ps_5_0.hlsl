...
// 8-bit UNORM in vanilla and are fp16 now, so the taps are taken per texel through the vanilla material curve and the
// output is saturated. Otherwise verbatim.
...
void main(float4 v0 : TEXCOORD0, float4 v1 : TEXCOORD1, float4 v2 : TEXCOORD2, float4 v3 : TEXCOORD3, float4 v4 : TEXCOORD4, float4 v5 : TEXCOORD5, float4 v6 : TEXCOORD6, float4 v7 : TEXCOORD7, out float4 o0 : SV_Target0)
{
   ...
   float3 mean = 0.0;
   for (int i = 0; i < 8; i++)
      mean += (SampleSaturatedBilinear(t0, s0_s, taps[i].zw, true).rgb + SampleSaturatedBilinear(t0, s0_s, taps[i].xy, true).rgb) * 0.0625;
