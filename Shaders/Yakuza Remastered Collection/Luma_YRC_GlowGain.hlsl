...
// The scene term uses this texel's footprint, like the prefilter. LumaData.CustomData1 != 0 = Y5R's material curve.
...
float4 glow_gain_ps(float4 pos : SV_Position) : SV_Target
{
   ...
   if ((cb11[0].y & 8u) != 0u)
   {
      ...
      glow += GlowSceneThreshold(pos.xy * texel, float2(texel.x * 0.2, 0.0), float2(0.0, texel.y * (1.0 / 6.0)), LumaData.CustomData1 != 0u);
   }
   glow = saturate(glow);
